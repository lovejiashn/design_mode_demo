package com.jiashn.designmode.prototypemode.service.impl;
...
import com.jiashn.designmode.prototypemode.utils.QuestionBank;
 ...
 */
@Service
public class PrototypeModeRandomQuestionServiceImpl implements PrototypeModeRandomQuestionService {

    private static final Logger logger = LoggerFactory.getLogger(PrototypeModeRandomQuestionServiceImpl.class);
    @Override
    public void randomQuestion() {
        try {
            logger.info("-----------------生成问卷1-------------------------------");
            logger.info(createQuestion("张晓明","s21090001"));
            logger.info("-----------------生成问卷2-------------------------------");
            logger.info(createQuestion("小飞飞","s21090002"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private String createQuestion(String candidate,String stuNum) throws Exception{
        QuestionBank cloneBank = (QuestionBank) questionBank().clone();
        cloneBank.setCandidate(candidate);
        cloneBank.setStuNumber(stuNum);
        return cloneBank.toString();
    }
